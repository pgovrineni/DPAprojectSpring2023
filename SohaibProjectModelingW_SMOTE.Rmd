...
creditcard['Amount_scaled']<-scale(creditcard$Amount)
creditcard['time_scaled']<-scale(creditcard$Time)

# remove unscaled amoyunt and time columns
creditcard<-creditcard[ -c(1,30) ]
...
train <- SMOTE(train[,-29],train$Class,dup_size = 310)
train<-train$data
prop.table(table(train$class))
...
#predict
logfit_pred<- predict(logfit, test[,-29],type ='response' )
...
# Create ROC curve
roc_score=roc(as.numeric(test$Class), as.numeric(logfit_pred)) #AUC score
